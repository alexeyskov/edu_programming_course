...
параллельных операций ограничено: три контекста для тяжёлых операций и отдельные
четыре лёгких контекста без JavaScript для запуска работ и пакетной сдачи Quiz.
Ожидающие студенты не занимают вкладки; импорт преподавателя не занимает лёгкие
слоты. Лимиты и нагрузочные оговорки описаны в `../DEPLOYMENT.md`.

Основные прикладные маршруты:
...
Для пакетной сдачи нескольких задач подготовленной попытки Quiz используется
`POST /internal/v1/moodle/quiz/answers/sync`: пакет ответов, привязанных к точным
номеру попытки и слотам вопросов. Он читает параметры текущего draft из штатной
HTML-формы и загружает файлы через обычный student-session upload endpoint Moodle,
без запуска filepicker/редактора. Затем штатные формы сохраняют страницы; перед
Finish коннектор перечитывает и проверяет по SHA-256 каждый ответ. Чужие файлы
не удаляются, перезапись требует подтверждённого прежнего файла и его хеша.
Изменения настроек Moodle и включение webservices для этого не нужны.
