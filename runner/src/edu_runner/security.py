...
def sign_body(secret: bytes, timestamp: str, nonce: str, body: bytes) -> str:
    """Return the exact value expected in X-Runner-Signature."""
    signature = hmac.new(
        secret,
...
def signed_headers(
    ...
    *,
    ...
    """Backend helper for the runner HMAC v1 HTTP contract."""
    timestamp_text = str(int(time.time()) if timestamp is None else timestamp)
    nonce_text = nonce or secrets.token_hex(16)
    return {
        "X-Runner-Timestamp": timestamp_text,
        "X-Runner-Nonce": nonce_text,
        "X-Runner-Signature": sign_body(secret, timestamp_text, nonce_text, body),
    }


class RequestAuthenticator:
    def __init__(
        self, secret: bytes, *, clock_skew_seconds: int, nonce_ttl_seconds: int
    ):
        self._secret = secret
    ...
    def verify(
        self, headers: Mapping[str, str], body: bytes, *, now: float | None = None
    ) -> None:
        timestamp = headers.get("x-runner-timestamp")
        nonce = headers.get("x-runner-nonce")
        signature = headers.get("x-runner-signature")
        if timestamp is None or nonce is None or signature is None:
            raise AuthenticationError("missing runner authentication headers")
        if not NONCE_PATTERN.fullmatch(nonce):
            raise AuthenticationError("invalid runner nonce")
        try:
            timestamp_value = int(timestamp)
        except ValueError as exc:
            raise AuthenticationError("invalid runner timestamp") from exc
        current = time.time() if now is None else now
        if abs(current - timestamp_value) > self._clock_skew:
            raise AuthenticationError("runner timestamp is outside the accepted window")
        expected = sign_body(self._secret, timestamp, nonce, body)
        if not hmac.compare_digest(signature, expected):
            raise AuthenticationError("invalid runner signature")
